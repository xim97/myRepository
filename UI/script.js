...
var articlesRenderer  = (function () {
    ...
    function renderHeaderAddButtom() {
        ...
        if (user != null) {
            ADD_ARTICLE_HOLDER.appendChild(ADD_ARTICLE_TEMPLATE.content.querySelector('.add-article-buttom').cloneNode(true));
            USER_NAME.textContent = user;
            HEADER_ACTIONS.querySelector('.login-logout-buttom').textContent = 'Выйти';
        } else {
            USER_NAME.textContent = 'Гость';
            HEADER_ACTIONS.querySelector('.login-logout-buttom').textContent = 'Войти';
        }
    }
    ...
    return {
        ...
        renderHeaderAddButtom: renderHeaderAddButtom,
        init: init
    };
}());
...
function startApp() {
    ...
    articlesRenderer.renderHeaderAddButtom();
    renderArticles();
}
